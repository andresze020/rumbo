 ...
 * as removable chips plus an inline searchable picker that can also **create a
 * new tag on the fly** (no need to leave a half-filled form), and emits the
 * selection as hidden inputs so a plain `<form action=…>` submits it: one
 * `tag_id` per selected tag, and a `tags_present` sentinel so the server action
 * can tell "cleared all tags" (present, empty) from "form didn't manage tags"
 * (absent). Inline (no portal) — it lives inside a scrollable dialog.
 */
          ...
          <div className="mt-2 max-h-48 overflow-y-auto">
            ...
            {query && !hasExact ? (
              <button
                ...
                className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-sm hover:bg-muted disabled:opacity-50"
              >

...
| BR-009 | P1 | Payees / merchants | **Partially resolved** — `payees` table exists and is backfilled from distinct `merchant_name` values per household (QA confirmed counts match after the sprint-13 `copy_household_data` test). Still missing, confirmed in QA: (1) no CRUD page to view/rename/merge/archive payees, (2) the transaction form has no select-existing-or-create-new payee picker, (3) nothing writes `payee_id` on create/edit yet. | Typos fragment reports and block rules/autocomplete; without a CRUD page the table is unmaintainable (no way to fix a bad backfilled name or merge duplicates). | First slice: payee picker (combobox: type to search existing, or create new) on the manual transaction form, wired to write `payee_id` on create/edit. Second slice: a minimal `/dashboard/payees` (or `/dashboard/settings`) CRUD list — rename, merge two payees into one, archive — once real usage shows which payees are noise. CSV import wiring and full autocomplete polish can follow. | Y | Manual entry picker (existing + new), edit keeps/changes payee, CSV import, merge keeps transaction history, household isolation. |
...
| BR-015 | P2 | Destructive-feeling actions | **Partially resolved** — added a reusable `AlertDialog` (`src/components/ui/alert-dialog.tsx`) and adopted it for void-transaction confirmation, replacing the old inline confirm state. Archive has no equivalent action yet (none exists today to standardize), and there is no undo/toast. | These actions are ledger-sensitive and can feel risky even when technically safe. | Adopt `AlertDialog` anywhere else a destructive confirm is inlined by hand; design and build an archive action (if/when it ships) on top of the same component; add undo/toast where practical. | N | Void, archive (once it exists), restore, cancel, keyboard/mobile. |
...
| BR-025 | P2 | Localization / currency | **First slice done** (PR #22, open, pending merge): ~20 files' duplicated `formatCurrency`/`formatMonthLabel`/label-casing helpers now import the shared `lib/format.ts` implementations instead of re-declaring their own — pure dedup, no behavior change. Still open: `formatPercent` duplicates diverge in digit-precision options per file (need per-site visual verification before centralizing); day-level date formatters have no shared helper; `lib/format.ts`'s `LOCALE` is still hardcoded `'en-CA'` regardless of the user's selected app language. | The app is bilingual and COP/CAD-heavy; formatting should match user expectations. | Second slice: audit and centralize `formatPercent` call sites one at a time (confirm digit options match before swapping), then decide whether `LOCALE` should follow the user's selected language. | N/~ | CAD/COP/USD display, ES/EN labels, mobile amount inputs. |
| BR-028 | P3 | PWA | **Done** (PR #21, open, pending merge): manifest `shortcuts` for Quick add / Transactions / Recurring; Quick add deep-links to `/dashboard?quick_add=expense`, which the transaction dialog provider now recognizes to auto-open the add-transaction dialog. No `share_target`. | Cheap mobile polish after core daily dashboard work. | Verify in a real installed PWA (long-press/right-click the home-screen icon) once merged — could not visually verify in the sandbox that built it (no Supabase session available). | N | Install manifest, icons, mobile browser behavior. |
...
4. BR-015 — Confirmation/undo pattern for void/archive (void done via `AlertDialog`; archive/undo still open).

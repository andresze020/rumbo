...
> (BR-030…BR-047, plus BR-048 raised during that sprint's QA).
...
| Field | Meaning |
|---|---|
| ID | Stable issue identifier for planning and follow-up. A trailing **✅** means shipped and closed; **🟡** means shipped in part — the remaining slice is listed in [../pending-work.md](../pending-work.md) §1. No marker means not started. |
...
> **Status as of 2026-08-16.** Every row in this table is closed except
> BR-021 🟡 (Close month is a soft marker by design — it does not lock the
> ledger) and BR-028 🟡 (shipped, but shortcuts and `share_target` have never
> been verified in a real installed PWA). The rows are kept for their "why soon"
> and "verify" columns, which are still the acceptance record.
...
> **Status as of 2026-08-16.** All four packages are closed. Kept as the
...
1. ✅ BR-008 — Transaction pagination and server-side filters (PR #37).
2. ✅ BR-011 — Review status and review queue (core in Sprint 4 + PR #17; the
   polish branch activated the nav entry and added bulk Flag / Mark unreviewed).
...
1. ✅ BR-009 — Payees/merchant normalization and autocomplete. Fully closed:
   `/dashboard/payees` CRUD, Base UI autocomplete, single merge and atomic
   `merge_payees_bulk`.
2. ✅ BR-010 — Categorization rules (PR #37; `/dashboard/rules`).
3. ✅ BR-022 — Reports hub by payee/category (`/dashboard/reports`).

### Package 4 — Planning depth

1. ✅ BR-018 — Budget rollover/carryover (migration `20260716140000`).
2. ✅ BR-019 — Goals.
3. ✅ BR-020 — Debt payoff projection (`/dashboard/debt-planner`).
4. ✅ BR-021 — Month in review: `lib/health/score.ts` is the real, documented
   formula (savings rate 65% + budget adherence 35%, shared by dashboard and
   month-review) and a **light** Close month shipped (`month_closures` marker +
   snapshot, reopenable). It does **not** lock the ledger — deliberate.
...
7. Manually test a COP account import in a CAD-base household. Still open —
   tracked as a QA gate in [../pending-work.md](../pending-work.md) §4.1.

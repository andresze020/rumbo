...
| BR-009 | P1 | Payees / merchants | ✅ **Resolved.** Slice 1 (migration `20260716120000`): payee picker on the manual transaction form (add/edit/inline/assistant) wired through `create_manual_transaction`/`update_manual_transaction` (`p_payee_name` + `get_or_create_payee`) to write `payee_id`, `merchant_name` kept in sync. Slice 2 (migration `20260717120000`): `/dashboard/payees` CRUD (list + usage stats, create, rename → propagates to linked `merchant_name`, archive/restore, **merge** duplicates via `merge_payees` reassigning transactions then archiving the source); the picker became a searchable Base UI Autocomplete combobox; a `payee_id` transactions filter (all-time default, "View transactions" per payee); and Payer/Payee context labeling. `payees.is_archived` + `get_payees_with_stats` RPC added. | (Historical) Typos fragment reports and block rules/autocomplete; without a CRUD page the table was unmaintainable. | **Remaining, deferred:** CSV import does not set `payee_id` yet; recurring templates carry no payee; no bulk merge (one source at a time). | Y | Manual entry picker (existing + new), edit keeps/changes payee, merge keeps transaction history, household isolation. |

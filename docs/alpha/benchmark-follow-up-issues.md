...
| BR-029 | P3 | Transactions filters | `/dashboard/transactions` has no "All time" preset — any link/visit without an explicit date range silently defaults to the current month. Sprint 13 QA hit this via the dashboard's needs-review link and worked around it with an ad-hoc wide date range (`2000-01-01`–`2099-12-31`) baked into that one link. | The silent month default is a general trap for any future cross-month link or saved filter, not just BR-012. | Add a real "All time" preset alongside "This month"/"Last 30d"/etc., and point the dashboard needs-review link at it instead of the date-range hack. | N | Preset selects all transactions regardless of month, works with other filters combined (review/type/account), mobile chip layout. |

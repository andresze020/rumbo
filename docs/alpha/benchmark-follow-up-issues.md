...
| BR-012 | P2 | Dashboard | **Partially resolved** — `/dashboard` now has a real recent-activity feed, but still has no needs-review count. | The app needs a daily landing loop, not just monthly summaries. | Add a needs-review count (count of `transactions.review_status = 'unreviewed'`) next to the existing recent-activity feed. | N | Empty state, populated state, links to filtered transactions, responsive cards. |

...
| RUM-001 | Instrumentar baseline y trazabilidad de performance | P0 | M | Ninguna | ✅ **Hecho 2026-09-21** (capa de servidor medida 2026-09-25 para `/dashboard`, B-5 cerrado). Ver [`performance-baseline.md`](./performance-baseline.md) |
...
| RUM-005 | Descomponer y optimizar carga del Dashboard | **✅ Hecho 2026-09-25** (orquestación + streaming 2026-09-21; Router Cache + invalidación 2026-09-25) | M/L | RUM-001, RUM-002 | Los 7 awaits secuenciales + los 5 en paralelo + `netWorthTrend` al final se unieron en un solo `Promise.all` de 13; se encontró y corrigió el mismo patrón N+1 de RUM-006 escondido en `trend-actions.ts` (6 llamadas a `get_account_balances` por mes → 1 a `get_account_balances_as_of_many`). Todo lo debajo del fold ahora streamea detrás de un `<Suspense>` (primer uso en el repo) en `src/app/dashboard/secondary-widgets.tsx`. Cierre 2026-09-25 (decisión del usuario): `staleTimes` 30 s del Router Cache, sin cache de servidor (RLS intacta); revisitas ~500–730 ms → ~80 ms; un test exige que cada Server Action que escribe invalide. Ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |
...
| RUM-004 | Optimizar consultas de Transactions | **✅ Hecho 2026-09-24** | M | RUM-001 | No era la query: las políticas RLS de `select` en 5 tablas llamaban `is_household_member()` (SECURITY DEFINER, nunca inlineada) por fila. Migración aplicada y verificada: `get_account_balances` 192 ms/36.778 buffers → 25 ms/3.024 buffers; `search_household_transactions` (all-time) 186 ms/34.791 buffers → 22 ms/1.731 buffers. Ningún índice nuevo ni keyset pagination hicieron falta. Ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |
| RUM-008 | Simplificar arquitectura de información del Dashboard | **✅ Hecho 2026-09-24** | M | RUM-002 | Alcance acotado a los 3 criterios de aceptación literales (label de mes dinámico, Budget/Goals consolidados en una tarjeta, Insights a 2) en vez de la jerarquía completa propuesta. Debts queda fuera de la consolidación a propósito. Ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |
| RUM-009 | Corregir semántica de Month health, Insights y secundarios | **✅ Hecho 2026-09-24** | M | RUM-002, RUM-008 | Misma fórmula, ahora con desglose visible (`healthBreakdown`) compartido por dashboard y Month review; Insights en módulo puro con tests; Debts distingue Debt Planner de liabilities de cuenta; Scheduled activity; review queue acotada al mes. Ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |
| RUM-010b | Suite de regresión, carga y release gate | **✅ Hecho 2026-09-24** | M/L | Todos | `npm run db:local` (invariantes sobre fixtures generadas, en CI) + `npm run perf:nav` + [`release-checklist.md`](./release-checklist.md). Primer veredicto: no aprobado por B-7; B-7 arreglado y B-8 aplicado el 2026-09-25 → **aprobado**. Ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |

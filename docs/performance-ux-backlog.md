...
| RUM-004 | Optimizar consultas de Transactions | **🟡 Causa raíz encontrada 2026-09-24, migración sin aplicar** | M | RUM-001 | No era la query: las políticas RLS de `select` en 5 tablas llaman `is_household_member()` (SECURITY DEFINER, nunca inlineada) por fila — con RLS real, 196 ms/Nested Loop; sin RLS, 27 ms/Hash Join, mismo household. Probablemente explica también el costo de `get_account_balances`. Migración redactada, bloqueada por permisos del harness al aplicar — ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |

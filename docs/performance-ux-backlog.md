...
| RUM-005 | Descomponer y optimizar carga del Dashboard | **🟡 Orquestación hecha 2026-09-21** (Suspense/streaming/cache pendientes) | M/L | RUM-001, RUM-002 | Los 7 awaits secuenciales + los 5 en paralelo + `netWorthTrend` al final se unieron en un solo `Promise.all` de 13; se encontró y corrigió el mismo patrón N+1 de RUM-006 escondido en `trend-actions.ts` (6 llamadas a `get_account_balances` por mes → 1 a `get_account_balances_as_of_many`). Streaming con `Suspense` y cache/invalidación quedan sin hacer — no dependen de RUM-002 pero son alcance nuevo, no orquestación. Ver [`performance-ux-execution-status.md`](./performance-ux-execution-status.md) |

...
| BR-009 | Slice 1 (payee picker, migration `20260716120000`) + Slice 2 (payees CRUD, branch `feat/br-009-payee-crud`, migration `20260717120000`). Slice 1: the transaction form's Merchant field is a Payee combobox wired through `create_manual_transaction` / `update_manual_transaction` (`p_payee_name` + `get_or_create_payee`) to write `payee_id` (merchant_name kept in sync); covers add/edit/inline/assistant forms. Slice 2: `/dashboard/payees` management page (list with usage stats, create, rename → propagates to linked `merchant_name`, archive/restore, and **merge** duplicates via `merge_payees` RPC that reassigns transactions then archives the drained source). Added `payees.is_archived` + `get_payees_with_stats` RPC + Money-group nav entry. | CSV import does not set `payee_id` yet; recurring templates carry no payee | [alpha/benchmark-follow-up-issues.md](./alpha/benchmark-follow-up-issues.md) |

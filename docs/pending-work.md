...
>
> **Touched 2026-08-21** to add the follow-ups left by the viewport-pinning
> sprint (§2 zoom decision, §3 two UI rows). That was an additive edit, not a
...
| **`supabase/tests/br_003_006_money_invariants.sql` had been silently unrunnable since it was written** | Every check that calls `get_account_balances` (`is_household_member()`-gated) failed outright under `npm run db:test`, because the runner connects as `postgres` with no JWT — `auth.uid()` reads null, so the guard always raised, regardless of role or RLS. Nobody could have run this file's balance checks successfully before. Fixed 2026-09-21 (RUM-006, alongside `get_account_balances_as_of_many`): `db-test.mjs --user=<uuid>` now runs each check `set local role authenticated` with that user's JWT claims. Once it could actually run, **`BR-006 official balances match posted/pending entries only` failed** — `get_account_balances(household)`'s `projected_balance_account_currency` disagrees with a manual sum of posted+pending `transaction_entries` for at least one account in the household tested. | Investigate with `rumbo-ledger-rules` in scope: run `npm run db:test -- --user=<uuid> --file=br_003_006` to reproduce, then read `get_account_balances(uuid)` (`20260817120000_balance_fx_revaluation.sql`) against the check's own manual-sum query (`br_003_006_money_invariants.sql`) to find where they diverge. Not investigated as part of RUM-006 — out of that ticket's scope, and it is a money-correctness question, not a performance one. |

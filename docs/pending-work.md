...
>
> **Last refreshed 2026-07-25.** Full English, Spanish, and Canadian French UI
> coverage is implemented. The language preference is persisted in
> `profiles.locale` and restored at login in new browsers or devices.
...
| ~~User Settings page (`/dashboard/settings`)~~ ✅ | **Shipped.** Profile, password, household, theme, language, and global sign-out are implemented. | — | [features/user-settings.md](./features/user-settings.md) |
...
| BR-025 | ✅ Full UI localization is implemented for `en`, `es`, and `fr`, including deep leaf components, daily dates, dynamic dialogs, loading states, and accessible labels. Coverage is enforced by `npm run i18n:check`. | Nothing outstanding. | [features/localization.md](./features/localization.md) |
